...
local state = {
    ...
    BUILD_TAG = 'acrylia-reduce-chain-2026-07-14-lw',   -- dev build marker; bump on every change (log header + Settings = stale-copy check)
    running = true,
    windowOpen = true,
    wasOpen = true,
    sizeSet = false,
...
local function all_recipe_names()
    local names = {}
...
state.reduceChains = {
    ...
    -- Acrylia ore: same four-form ladder as velium. Drops come in any size, so we chisel down (reduce)
    -- when a bigger form is on hand and build up (the natural-name recipes) otherwise. The `Reduce ...`
    -- sections keep the reduce recipes from colliding with the build recipes of the same output form.
    ['Small Piece of Acrylia']     = { section = 'Small Piece Of Acrylia',            from = 'Small Brick of Acrylia Ore' },
    ['Small Brick of Acrylia Ore'] = { section = 'Reduce Small Brick Of Acrylia Ore', from = 'Large Brick of Acrylia Ore' },
    ['Large Brick of Acrylia Ore'] = { section = 'Reduce Large Brick Of Acrylia Ore', from = 'Block of Acrylia Ore' },
}
...
local function build_dropped_indexes()
    _droppedBySkill = {}
    local allSet = {}
...
local function open_merchant(vendorName)
    ...
    -- A leftover forge/oven/combine window blocks the merchant from opening -- this
    -- was the cause of the vendor hangs. Clear any station windows before trying.
    close_station_windows()
    for _ = 1, 3 do
        check_stop()
        ...
        if merchant_open() then
            -- Open, THEN wait (briefly) for the list to populate before returning. Returns the
            -- instant the count reads nonzero; capped at ~2.5s so a vendor whose count never
            -- populates doesn't stall us - buy_item's short retry is the backup.
            delay(2500, function() return (mq.TLO.Merchant.Items() or 0) > 0 end)
            -- Turn OFF "Show only items I can use". Checked, it hides non-equippable tradeskill
            -- mats from the visible list the scanner walks. It's a button (no writable state), so
            -- we read .Checked and click to uncheck ONLY when it's on - never flipping it back on.
...
local function return_cursor_items(cinfo, rec, kitPack)
    ...
    while cursor_id() > 0 and guard < 40 do
        ...
        if cstack == 1 and isReturnedTool[cname] then
            ...
            returned = true
        elseif cstack == 1 and (need[cname] or 0) > kit_qty(cname) then
            local slot = first_free_kit_slot(kitPack)
            if slot then
                mq.cmdf('/nomodkey /itemnotify in pack%d %d leftmouseup', kitPack, slot)
                mq.delay(800, function() return cursor_id() == 0 or (mq.TLO.Cursor.Name() or ''):upper() ~= cname end)
                returned = true
            end
        end
        if not returned then
            mq.cmd('/autoinventory')  -- product, or a returned stack -> bags
            mq.delay(700, function() return cursor_id() == 0 or (mq.TLO.Cursor.Name() or ''):upper() ~= cname end)
        end
        if desyncDetected then
...
local function buy_pass(rec, quantity, defaultVendor)
    ...
    -- Close any open tradeskill container before visiting vendors.
    close_station_windows()
    ...
    local function pick_vendor(itemName)
        ...
        local function reachable(inst, allowThurg)
            ...
            if z == curZone or z == ZONE_POK or z == ZONE_MARR or z == ZONE_JAGGEDPINE then
                return true
            end
...
local function run_research_engine(job)
    ...
    local ok, err = pcall(function()
        ...
        for pass = 1, SAFETY_PASSES do
            ...
            -- Bank is a first-class supplier: pull banked stock into bags FIRST, so the buy pass
            -- below (which nets on-hand) only buys the true shortfall - same as run_engine does.
            for nm, q in pairs(combined) do
                local short = q - item_count(nm)
                if short > 0 then
                    local banked = state.bank_count(nm)
                    if banked > 0 then
                        local got = state.withdraw_count(nm, math.min(short, banked))
                        if got > 0 then printf_log('Bank: pulled %d x %s for the research batch (skipping buy).', got, nm) end
                    end
                end
            end
            -- Done pulling: close the bank before we walk to vendors (withdraw_count leaves it open).
...
state.bankTopUp = function(itemName, target)
    ...
    if state.spawn_dist3d(mq.TLO.Target) > 10 or state.spawn_zgap(mq.TLO.Target) > 12 then
        ...
        -- Approach the banker from the safe waypoint first (its cluster wedges a head-on nav).
        -- Inside the >10 guard, so a multi-item bank visit routes through the hub once, not per item.
        do
    ...
    if shouldDraw then
        ...
        local ok, drawErr = pcall(function()
            ...
            if ImGui.BeginTabBar('##ts_tabs') then
                ...
                -- ── LEVELING TAB ─────────────────────────────────────────────────
                if ImGui.BeginTabItem('Leveling##ts_tab3') then
                    local _tok, _terr = pcall(function()
                    ...
                    if state.recPathSelected == 'Welcome' then
                        ...
                        ImGui.TextWrapped('Pottery needs caster-summoned mats. If your Enchanter and Cleric are in Marr\'s with enough platinum, they\'ll buy the reagents and summon these. No enchanter? Ask a friendly one for the totals below. Lazcraft grabs them as needed once you start Pottery - you can also queue them under Supply.')
                        ImGui.Spacing()
                        ...
                        if ImGui.BeginTable('##ts_rs_split', 2, 0, 0, 0) then
                            ...
                            if themed_button((armed and 'Confirm Start' or 'Start') .. '##ts_rs_qstart',
                                ...
                                if armed then
                                    ...
                                    local items = {}
                                    for _, e in ipairs(state.rsQueue) do
                                        items[#items + 1] = { name = e.name, key = e.name .. '##' .. e.class, qty = e.qty }
                                    end
                            ...
                            if qn == 0 then
                                ...
                                for i, e in ipairs(state.rsQueue) do
                                    ...
                                    ImGui.SameLine()
                                    if is_research_tome(e.name) then
                ...
                if ImGui.BeginTabItem('Supply##ts_tab_req') then
                    ...
                    -- ===== Made by casters (collapsed by default to de-clutter the tab) =====
                    if ImGui.CollapsingHeader('Made by Casters##ts_req_makehdr') then
                    ImGui.TextDisabled('Make: producer (in group) buys & casts a batch, keeps it, notifies you.')
                    ImGui.TextDisabled('Bring: producer delivers what it already made (no casting).')

                    -- Casters / Priests picker. Cosmetic labels only - smart-divide picks the real
                    -- producer class per item. "Casters" shows the Enchanter-made supply groups;
                    -- "Priests" shows the deity gem imbues.
                    if not state.reqMakeGroup then state.reqMakeGroup = 'Casters' end
                    if ImGui.BeginCombo('Type##ts_req_makeclass', state.reqMakeGroup) then
                        for _, c in ipairs({ 'Casters', 'Priests' }) do
                            if ImGui.Selectable(c .. '##ts_req_mkcls_' .. c, state.reqMakeGroup == c) then
                    ...
                    state.reqMakeQtyBuf = ImGui.InputText('Qty##ts_req_makeqty', state.reqMakeQtyBuf or '100', 8)
                    local makeQty = math.max(1, math.floor(tonumber(state.reqMakeQtyBuf) or 100))

                    -- Render one collapsible group of makeables (collapsed by default). Preserves the
                    -- Make / Bring / (Request for) buttons exactly as before.
                        ...
                        if ImGui.CollapsingHeader(label .. '##' .. hdrId) then
                            for i, m in ipairs(MAKEABLE) do
                                if m.group == groupKey then
                                    if ImGui.Button('Make##ts_req_makeadd_' .. i) then
                                        request_queue_add(m.item, 'make', makeQty)
                                    end
                                    ImGui.SameLine()
                                    if ImGui.Button('Bring##ts_req_bring_' .. i) then
                                        request_queue_add(m.item, 'stack')
                                    end
                                    ImGui.SameLine()
                                    ImGui.Text(m.item)
                                    -- Read-only class tags on gems, so a player sees at a glance which of
                                    -- their classes can make it (a Druid-only box knows its gems without
                                    -- guessing). Make/Bring are unchanged - this is purely informational.
                                    -- (All) = every priest class (Cleric+Druid+Shaman); otherwise list them.
                    ...
                    end  -- Made by Casters collapsing header
                    ...
                    if #state.requestQueue == 0 then
                        ImGui.TextDisabled('Queue empty. + items above, set All or a Quantity, then Run All.')
                    else
                        for i, e in ipairs(state.requestQueue) do
                            ImGui.Text(string.format('%d.', i))
                            ImGui.SameLine()
                            if e.mode == 'make' then
                                ImGui.TextColored(0.85, 0.70, 1.0, 1.0, string.format('Make x%d', e.qty or 0))
                                ImGui.SameLine()
                                ImGui.Text(e.item)
                            else
                                -- [All] + Quantity. All is lit while the box is empty/0; type a number
                                -- and it greys out (that exact count is used instead). This is the whole
                                -- source model now - everything comes from the group.
